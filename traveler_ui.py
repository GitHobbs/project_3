...
from cruise_line_ui import load_contract
...
df_sailings = pd.DataFrame(result, columns=['Sailing ID', 'Cruise Line', 'Cruise Name', 'Ship Name', 'Departure Date', 'Departure Port', 'Number of Nights', 'Destination1', 'Destination2', 'Destination3', 'Price(ETH)'])
...
df_cabins = pd.DataFrame([cabin], columns=['Cabin Type', 'Price', 'Availability', 'Sailing ID'])
st.table(df_cabins)
